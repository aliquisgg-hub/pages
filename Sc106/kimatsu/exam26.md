---
title: "Sc106(1) Bクラス 期末試験 (2026)"
last_modified: 2026-07-16 20:09:01 +09:00
...
---
<!-- Edge で 120% で印刷? -->
# <span style="font-size:80%;">Sc106(1) 数学入門 (三浦クラス) 期末試験 (2026)</span>
...
1. 以下の集合 $A～G$ について，下の問の空欄を適切に埋めなさい．
   なお，普遍集合 $\mathit Ω$ は $\boldsymbol N$ とします．
   $$
   \left\{\begin{split}
   A&=\{x\,|\,x は奇数\}\\       %{1,3,5,7,9, …}
   B&=\{x\,|\,x は3 の倍数\}\\   %{3,6,9, …}
   C&=\{x\,|\,x は素数\}\\       %{2,3,5,7, …}
   D&=\{x\,|\,2<x<11\}\\         %{3,4,5,6,7,8,9,10}
   E&=A^c∩D\\                   %=D\A={4,6,8,10}
   F&=D∖B^c\\                    %{3,6,9}
   G&=C∩D\\                     %{3,5,7}
   H&=(C∖B)∩D\\                 %{5,7}
   \end{split}\right.
   ...
   -  以下の集合の要素を列挙して $(\{1,2,3\}\ のように)$ 表しなさい．
      $$\begin{split}
      E&=\boxed{\rule[-1ex]{0pt}{3ex}\hspace{12em}}\qquad      %{4,6,8,10}
      F =\boxed{\rule[-1ex]{0pt}{3ex}\hspace{12em}}\\          %{3,6,9}
      G&=\boxed{\rule[-1ex]{0pt}{3ex}\hspace{12em}}\qquad      %{3,5,7}
      H =\boxed{\rule[-1ex]{0pt}{3ex}\hspace{12em}}\\          %{5,7}
      (B∖A)∩D&=\boxed{\rule[-1ex]{0pt}{3ex}\hspace{12em}}\\   %{6}
      \end{split}\hspace{6em}$$
   -  $A～H$ のうち，$G$ を部分集合として含むものを，全て列挙しなさい． <!-- A,C,D,G -->
      $\boxed{\rule[-1ex]{0pt}{3ex}\hspace{12em}}$
   -  $A～H$ のうち，$H$ と互いに素なものを，全て列挙しなさい．         <!-- B,E,F -->
      $\boxed{\rule[-1ex]{0pt}{3ex}\hspace{12em}}\rule{0pt}{3.5ex}$
   -  集合 $H$ の要素数は $\boxed{\rule[-1ex]{0pt}{3ex}\hspace{3em}}$ であり，集合 $E$
      と集合 $F$ の直積集合の要素数は $\boxed{\rule[-1ex]{0pt}{3ex}\hspace{3em}}\rule{0pt}{3.5ex}$ である． <!-- 3 -->
   -  集合 $G$ の全ての部分集合の数は $\boxed{\rule[-1ex]{0pt}{3ex}\hspace{3em}}$ である．  
      &nbsp;
   <!-- 
...
1. 集合 $D=\{x\,|\,x∈\boldsymbol R,\ x≧0\}$ とするとき，以下の式で表される $D$ から $D$ への写像 $f,g,h$ について，下の問に答えなさい．
   $$
   \begin{split}
   f(x)&=2x\\
   g(x)&=x^2+1\\
   h(x)&=x^3-2x^2+x\ (=x(x-1)^2)
   \end{split}
   $$
   -  写像 $f,g,h$ のうち，1対1写像（単射）を全て列挙しなさい．         <!-- f,g -->
      $\boxed{\rule[-1ex]{0pt}{3ex}\hspace{12em}}$
   -  写像 $f,g,h$ のうち，上への写像（全射）を全て列挙しなさい．       <!-- f,h -->
      $\boxed{\rule[-1ex]{0pt}{3ex}\hspace{12em}}\rule{0pt}{3.5ex}$
   -  写像 $f,g,h$ のうち，1対1上への写像（全単射）を全て列挙しなさい． <!-- f -->
      $\boxed{\rule[-1ex]{0pt}{3ex}\hspace{12em}}\rule{0pt}{3.5ex}$
   ...
   -  以下の写像を，$x$ を用いた括弧を含まない式で表しなさい．
      $$
      \begin{split}
      f\circ g(x) = f(g(x)) = \boxed{\rule[-2ex]{0pt}{5ex}\hspace{16em}}\\ % 2x^2+2
      g\circ f(x) = g(f(x)) = \boxed{\rule[-2ex]{0pt}{5ex}\hspace{16em}}\\ % 4x^2+1
      f^{-1}(x)=\boxed{\rule[-2ex]{0pt}{5ex}\hspace{16em}}                 % x/2
      \end{split}
      $$  
...
1. 命題と論理式に関して，下の問に答えなさい．なお，必要ならば，括弧や以下の記号を用いてもよい．  
   　　　　$￢$：「～でない」，$∧$：「かつ」，$∨$：「または」，$⇒$：「ならば」，  
   ...
   -  $P =「a は偶数である」$，$Q =「3a は偶数である」$とします．
      ...
      -  $(A)$ の論理式の逆と対偶と否定を $P,Q$ を用いた論理式で表しなさい．ただし，否定は $⇒$ を使用しないこと．  
...
1. 以下の二項演算のうち，*閉じていない*ものを全て選び，$a～h$ の記号で列挙しなさい．
   ...
   $(f)\ (\boldsymbol Z∖\{0\};\ \div)$： 0以外の整数の割り算  
   $(g)\ (\boldsymbol Q;\ \times)$： 有理数の掛け算$\hspace{2.1em}$
   $(h)\ (\boldsymbol Q∖\{0\};\ \div)$： 0以外の有理数の割り算  
   &nbsp;
...
1. $\vec{\vphantom{b}a}=(2,1)$，$\vec b=(-1,2)$ として，以下の各々の計算結果を空欄に記しなさい．
...
1. ベクトルと行列について，以下の各々の計算結果を空欄に記しなさい．
   ...
   $\begin{pmatrix}1&2\\4&3\end{pmatrix}\begin{pmatrix*}[r]4&3\\1&-2\end{pmatrix*}
   =\boxed{\rule[-3ex]{0pt}{7ex}\hspace{6em}}$，　
   $2\cdot\begin{pmatrix}1&2\\4&3\end{pmatrix}-
   \begin{pmatrix*}[r]4&3\\1&-2\end{pmatrix*}=\boxed{\rule[-3ex]{0pt}{7ex}\hspace{6em}}$，  
   $\begin{pmatrix}1&2\\4&3\end{pmatrix}\begin{pmatrix}2\\1\end{pmatrix}
...
1. 7つの数値 $2,2,2,3,5,6,8$ について，以下の代表値を求めて空欄に記しなさい．  
   平均値：$\boxed{\rule[-1ex]{0pt}{3ex}\hspace{4em}}$，　
   中央値：$\boxed{\rule[-1ex]{0pt}{3ex}\hspace{4em}}$，　
   最頻値：$\boxed{\rule[-1ex]{0pt}{3ex}\hspace{4em}}$．  
   &nbsp;

1. <img style="float:right;margin-top:2ex; margin-right:4em;" src="data26.svg">
